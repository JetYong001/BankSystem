...
import views.customerPage.Transfer.TransferView;
...
public class Customer_Dashboard extends JPanel implements Themeable {
    ...
    public Customer_Dashboard(String customerUsername) {
        setLayout(new BorderLayout());
        initGlobalTimer();

        Customer currentUser = UserDAO.getCustomerByUsername(customerUsername);
        boolean isDark = UserDAO.getThemePreference(customerUsername);
        ...
        pages.put("Account", new DashboardView(currentUser,isDark));
        pages.put("Transfer", new TransferView(currentUser,isDark));
        pages.put("Deposit/Withdrawal", new WithdrawDepositView(currentUser,isDark));
        pages.put("Card", new CardView(currentUser,isDark));
        pages.put("Application", new ApplyView(currentUser,isDark));
        pages.put("Transaction View", new TransactionView(currentUser,isDark));
        pages.put("Interest Rate Calculation", new InterestRateCalculatorView(currentUser,isDark));
        assert currentUser != null;
        pages.put("Profile", new CustomerProfile(currentUser,isDark));
        pages.put("Setting", new SettingsView(pages, customerUsername));

        ArrayList<String> menu = new ArrayList<>();
        menu.add("Account");
        menu.add("Transfer");
        menu.add("Deposit/Withdrawal");
        menu.add("Card");
        menu.add("Application");
        menu.add("Transaction View");
        menu.add("Interest Rate Calculation");
        menu.add("Profile");
        menu.add("Setting");
        ...
        for (String item : menu) {
            ...
            btn.addActionListener(e -> {
                ...
                Customer freshUser = UserDAO.getCustomerByUsername(customerUsername);

                if ("Account".equals(item)) {
                    DashboardView dv = (DashboardView) pages.get("Account");
                    if (dv != null) {
                        dv.refreshData(freshUser);
                    }
                } else if ("Transaction View".equals(item)) {
                    TransactionView tv = (TransactionView) pages.get("Transaction View");
                    if (tv != null) {
                        tv.loadData(freshUser);
                    }
                }
    ...
    public void updateTheme(boolean isDarkMode) {
        applyThemeToContainer(contentPanel, isDarkMode);
        repaint();
        revalidate();
    }
